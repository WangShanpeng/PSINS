function [posdr,posdr0,posdr1] = PipeLineC(imuod, t, pos, yaw0, inst, kod, Td)
% 输入参数：
...
%   pos = [pos0; pos1]  pos0 起始位置（纬经rad，高m）; pos1 校正点位置
...
%   posdr0,posdr1 = 往、返程位置
    ...
    avp = drpure(datacut(imuod,t0,t1), [att;pos0], inst, kod, Td); close(gcf); % insplot(avp);
%     pos1DR = getat(avp,t1);
    idx = posnearest(avp(:,7:10), pos1);  pos1DR = avp(idx,:)';    
    [inst1, kod] = drcalibrate(avp(1,7:9)', pos1, pos1DR(7:9));
    avp = drpure(datacut(imuod,t0,t2), [att-inst1;pos0], inst, kod, Td); % insplot(avp);
    %%
    pos11 = datacut(avp(:,[7:9,end]),1,t1);  idx1 = posnearest(pos11, pos1);
    posdr0 = drfit(pos11, pos0, [pos1;idx1], 1);  title('forward fit');
    pos22 = datacut(avp(:,[7:9,end]),t1,t2);
    posdr1 = drfit(pos22, posdr0(end,1:3)', pos0, 1);  title('backward fit');
