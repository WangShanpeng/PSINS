...
{
	CVect3 wm, vm, eb=CVect3(-4,1.3,0)*glv.dps, db=O31;
	CMahony mahony(10.0);
	CFileRdWt::Dir(".\\Data\\");  CFileRdWt fmh("mahony.bin");
	CConUart uart;
	uart.Init(3, 460800, 35*4, 0xaa55);  // COM setting
	for(int i=0; i<5*3600*100; )
	{
		if(uart.getUart())
		{
			uart.dispUart();
			wm = CVect3(uart.ps->gx, uart.ps->gy, uart.ps->gz)*glv.dps*TS10 - eb*TS10;
			vm = CVect3(uart.ps->ax, uart.ps->ay, uart.ps->az)*glv.g0*TS10 - db*TS10;
			mahony.Update(wm, vm, 0.01);
			if(i++%21==0) {
				uart.gotorc(20,60);
				CVect3 att=q2att(mahony.qnb);
				printf("Mahony-Att: %8.3f, %8.3f, %8.3f", att.i/DEG, att.j/DEG, att.k/DEG);
				uart.gotorc(21,60);
				printf("Mahony-eb:  %8.3f, %8.3f, %8.3f", mahony.exyzInt.i/DPS, mahony.exyzInt.j/DPS, mahony.exyzInt.k/DPS);
				if(fmh.f) fmh<<mahony;
			}
			if(i%100==0) fflush(fmh.f);
		}
	}
}
