function [clbt, av] = sysclbtKap(imu, pos0, g0, Cba, itertion, isKap)
% SIMU systemtic calibration processing under specific rotating operation.
%
% Prototype: [clbt, av] = sysclbtKap(imu, pos0, g0, Cba, itertion)
...
%               inculde fields:
%                 'Kg, Ka, eb, db, Kap, rx, ry, rz, tGA', such that
%                 wm = Kg*wm - eb*ts
%                 vm = Ka*vm - (db+Kap.*abs(fb)+fL+tGA*wbXfb)*ts
%                where wb=wm/ts, fb=vm/ts, and fL is accelerometer inner
%                lever arm effect (refer to the following code). 
%
% See also  sysclbt, sysclbtMEMS, imuscale, imudpdrift, lsclbt, clbtfile, clbtdiff, imuclbt, imuerrset, kfupdate.

% Copyright(c) 2009-2016, by Gongmin Yan, All rights reserved.
% Northwestern Polytechnical University, Xi An, P.R.China
% 17/08/2016
global glv
    if nargin<6, isKap=0; end
    ...
    clbt.Kg = eye(3); clbt.Ka = eye(3); clbt.Kap = zeros(3,1); clbt.eb = zeros(3,1); clbt.db = zeros(3,1);
...
% kf = clbtkfinit(nts);
        ...
        for k=2*frq2:nn:len-frq2
            ...
            fn = qmulv(qnb, fb-clbt.Kap.*abs(fb)-fL-clbt.tGA*cross(wb,fb));
...
global glv
    ...
    kf.Pxk = diag([ [0.1;0.1;1]*glv.deg; [1;1;1]; [1;1;1]*0.1*glv.dph; [1;1;1]*glv.mg; ...
        ...
        [100*glv.ppm;100*glv.sec;100*glv.sec]; [  0*glv.sec;100*glv.ppm;100*glv.sec]; [  0*glv.sec;  0*glv.sec;100*glv.ppm]; [1;1;1]*100*glv.ppm; ...
...
function Ft = getFt(fb, wb, Cnb, wnie, SS)   % kffk
    ...
    CDpf = Cnb*diag(abs(fb)); CwXf = Cnb*cross(wb,fb);
    %        1   4     7    10    13       16       19       22       25       28       31    34  37  40   43    
    %states: fi  dvn   eb   db    dKg(:,1) dKg(:,2) dKg(:,3) dKa(:,1) dKa(:,2) dKa(:,3) dKap  rx  ry  rz   tGA
    Ft = [  -wX  o33  -Cnb  o33  -wx*Cnb  -wy*Cnb  -wz*Cnb   o33      o33      o33      o33   o33 o33 o33  o31
             fX  o33   o33  Cnb   o33      o33      o33      fx*Cnb   fy*Cnb   fz*Cnb   CDpf  Cnb*SS       CwXf
             zeros(37,43) ];
