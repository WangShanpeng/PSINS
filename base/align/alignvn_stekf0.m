function [att0, attk, xkpk] = alignvn_stekf0(imu, qnb, pos, phi0, imuerr, wvn, isfig)
...
global glv
    ...
    for k=1:nn:len-nn+1
        ...
        set0 = 0;
        kf.Ft(1:3,1:9) = [-askew(eth.wnin)+Fev*askew(vn)*set0, Fev*set0, -Cnb];  
        kf.Ft(4:6,1:12) = [-askew(eth.gn)-askew(vn)*askew(eth.wnie)*set0, -askew(2*eth.wnie+eth.wnen)*set0, askew(vn)*Cnb*set0, Cnb]; 
        kf.Phikk_1 = kf.I + kf.Ft * nts;
        kf.Hk(:,1:3) = askew(vn)*set0;
        kf = kfupdate(kf, vn);
