...
%% EKF ***********************************************************
zint = fix(1/Ts);  % measurement interval/frequency
res = zeros(len,2*n);
Qk = zeros(n);  Rk = rk^2;
Xk0 = [x10;x20;0]+[1000;600;0.001].*randn(3,1);  Pk0 = diag([1000,600,0.001])^2; % init KF X0,P0
initSmall=1;  if initSmall==1,  Xk0(3) = 0.001+0.0001*randn(1);  Pk0(3,3) = 0.0001^2;  end  % ***
Xk = Xk0;  Pk = Pk0;
for k=1:len
...
for k=1:len
    ...
    for k1=1:L, dX = vfbfx(Xk+sP(:,k1)*d,tpara)-vfbfx(Xk-sP(:,k1)*d,tpara); Pkk_1 = Pkk_1 + dX*dX'/(4*d^2); end; 
    if mod(k,zint)==0
        Zkk_1 = vfbhx(Xkk_1, tpara);
        sP = chol(Pkk_1,'lower');  PXZ = zeros(n,m); PZZ = Rk;
        for k1=1:L
            dX = sP(:,k1); dZ = vfbhx(Xkk_1+sP(:,k1)*d,tpara)-vfbhx(Xkk_1-sP(:,k1)*d,tpara);
            PXZ = PXZ + dX*dZ'/(2*d); PZZ = PZZ + dZ*dZ'/(4*d^2);
        end;
        Kk = PXZ*PZZ^-1;
        Xk = Xkk_1 + Kk*(zk(k)-Zkk_1);
        Pk = Pkk_1 - Kk*PZZ*Kk';  Pk = (Pk+Pk')/2;
...
for k=1:len
    ...
    for k1=1:L, Xp(:,k1) = vfbfx(Xk+sP(:,k1)*d,tpara); Xn(:,k1) = vfbfx(Xk-sP(:,k1)*d,tpara); Xkk_1 = Xkk_1+Xp(:,k1)+Xn(:,k1); end;
    Xkk_1 = (d^2-n)/d^2*X0 + Xkk_1/(2*d^2);
    P1 = zeros(n); P2 = P1;
    for k1=1:L, dX = Xp(:,k1)-Xn(:,k1); P1 = P1+dX*dX'; dX2 = Xp(:,k1)+Xn(:,k1)-2*X0; P2 = P2+dX2*dX2'; end;
    Pkk_1 = P1/(4*d^2) + P2*(d^2-1)/(4*d^2) + Qk;  
    if mod(k,zint)==0
        sP = chol(Pkk_1,'lower');
        Z0 = vfbhx(Xkk_1, tpara); Zp = zeros(m,n); Zn = Zp; Zkk_1 = zeros(m,1);
        for k1=1:L, Zp(:,k1) = vfbhx(Xkk_1+sP(:,k1)*d,tpara); Zn(:,k1) = vfbhx(Xkk_1-sP(:,k1)*d,tpara); Zkk_1 = Zkk_1+Zp(:,k1)+Zn(:,k1); end;
        Zkk_1 = (d^2-n)/d^2*Z0 + Zkk_1/(2*d^2);
        PXZ = zeros(n,m); P1 = zeros(m); P2 = P1;
        for k1=1:L, dZ = Zp(:,k1)-Zn(:,k1); P1 = P1+dZ*dZ'; dZ2 = Zp(:,k1)+Zn(:,k1)-2*Z0; P2 = P2+dZ2*dZ2'; PXZ = PXZ+sP(:,k1)*dZ'; end;
...
for k=1:len
    ...
    Pkk_1=zeros(n);  for k1=1:L, dX = vfbfx(Xk+sP(:,k1)*d,tpara)-Xkk_1; Pkk_1 = Pkk_1+dX*dX'; end;
    Pkk_1 = Pkk_1/(2*d^2) + Qk;  
    if mod(k,zint)==0
        Zkk_1 = vfbhx(Xkk_1, tpara);
        sP = chol(Pkk_1,'lower');  sP = [sP, -sP];
        PXZ=zeros(n,m); PZZ=zeros(m);
        for k1=1:L, dX = sP(:,k1); dZ = vfbhx(Xkk_1+sP(:,k1)*d,tpara)-Zkk_1; PXZ = PXZ+dX*dZ'; PZZ = PZZ+dZ*dZ';  end;
...
%% SR-UKF ********************************************************
[U, wm, wc] = utpoint(n);  swc = sqrt(abs(wc));
...
for k=1:len
    ...
    Xkk_1=zeros(n,1);  for k1=1:L, Xi(:,k1) = vfbfx(Xi(:,k1),tpara); Xkk_1 = Xkk_1+wm(k1)*Xi(:,k1); end;  % mean
    dX=zeros(n,L);  for k1=1:L, dX(:,k1) = swc(k1)*(Xi(:,k1)-Xkk_1); end;
    [q,sPkk_1] = qr([dX(:,1:L-1), sQk]',0);  sPkk_1 = mycholupdate(sPkk_1,dX(:,end),-1)';  % var
    if mod(k,zint)==0
        sP = gmm*sPkk_1;  dX = [sP, -sP, zeros(n,1)];  Xi = dX+repmat(Xkk_1,1,L);  XiZ = zeros(m,L);
        Zkk_1=zeros(m,1);  for k1=1:L, XiZ(:,k1) = vfbhx(Xi(:,k1),tpara); Zkk_1 = Zkk_1+wm(k1)*XiZ(:,k1); end;
        PXZ=zeros(n,m); dZ=zeros(m,L);  for k1=1:L, dZ(:,k1) = swc(k1)*(XiZ(:,k1)-Zkk_1); PXZ = PXZ+swc(k1)*dX(:,k1)*dZ(:,k1)'; end;
        [q,sPZZ] = qr([dZ(:,1:L-1), sRk]',0);  sPZZ = mycholupdate(sPZZ,dZ(:,end),-1)';
        Kk = PXZ/(sPZZ*sPZZ');
        Xk = Xkk_1 + Kk*(zk(k)-Zkk_1);
...
nextlinestyle(-1); xx=[]; kk = 1:14; 8:11; [5,7,10,13]; [10,8,3,12,6,2,5,7,1]; 
for k=kk
