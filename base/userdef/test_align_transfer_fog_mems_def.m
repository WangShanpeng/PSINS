function out = test_align_transfer_fog_mems_def(tag, varargin)
% See also  test_align_transfer_fog_mems, kfinit, kffk, kfhk, kfplot.
global glv psinsdef
switch tag
	case psinsdef.kfinittag,
        ...
        kf.Pxk = diag([[10;10;10]*glv.deg; [10;10;10]; imuerr.eb; imuerr.db; ...
                    [1;1;1]*glv.deg; [1;1;1]/10*glv.deg; [10;10;10]*glv.dps])^2;
        out = kf;
    case psinsdef.kffktag,
    case psinsdef.kfhktag,
    case psinsdef.kfplottag,
        [xkpk, res, eb0, db0, mu0] = setvals(varargin);
        ...
        subplot(423),plot(t, xkpk(:,7:9)/glv.dph), xygo('eb'),  plot(t,repmat(eb0,1,len)/glv.dph,'-.'),
        subplot(424),plot(t, xkpk(:,10:12)/glv.ug), xygo('db'), plot(t,repmat(db0,1,len)/glv.ug,'-.'),
        subplot(425),plot(t, xkpk(:,13:15)/glv.min), xygo('mu'),plot(t,repmat(mu0,1,len)/glv.min,'-.'),
        subplot(426),plot(t, xkpk(:,16:18)/glv.min), xygo('theta'), 
        subplot(427),plot(t, xkpk(:,19:21)/glv.dps), xygo('w'), 
        myfigure,
