function out = test_SINS_GPS_tightly_def(tag, varargin)
% See also  test_GPS_INS_static_tight, kfinit, kffk, kfhk, kfplot.
global glv psinsdef
switch tag
	case psinsdef.kfinittag,
        [ins, davp, imuerr] = setvals(varargin);
        ...
        kf.Rk = diag([10/glv.Re;10/glv.Re;30])^2;
...
%         kf.adaptive = 1;
    ...
    case psinsdef.kfplottag,
        ...
        insplot(avp);   % avp
        subplot(323), plot(avpgps(:,10)-avpgps(1,10), avpgps(:,4:6), 'm');
        subplot(325), plot(avpgps(:,10)-avpgps(1,10), [[avpgps(:,7)-avp(1,7),(avpgps(:,8)-avp(1,8))*cos(avp(1,7))]*glv.Re,avpgps(:,9)-avp(1,9)], 'm');
        subplot(3,2,[4,6]), plot((avpgps(:,8)-avp(1,8))*glv.Re*cos(avp(1,7)), (avpgps(:,7)-avp(1,7))*glv.Re, 'm');
...
%         	plot(avpgps(:,10),avpgps(:,12), 'm');
        subplot(222), plot(xkpk(:,end), sqrt(xkpk(:,41))); xygo('std-dt(m)');
        subplot(224), plot(xkpk(:,end), sqrt(xkpk(:,42))); xygo('std-dtDot(m/s)');
end
