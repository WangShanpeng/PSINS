function [att0, res] = aligni0fitv(imu, pos, isfig)
% SINS initial align based on inertial frame method & using velocity curve fit.
%
% Prototype: [att0, res] = aligni0fitv(imu, pos, isfig)
% Inputs: imu - IMU data
%         pos - position
%         isfig - figure flag
...
%     [att0, res] = aligni0fit(imu, glv.pos0);
%     
% See also  aligni0, alignfn, alignvn, aligncmps, alignWahba, alignsb, aligni0fitp.
...
global glv
    ...
    [pib0k, pi0k, vi0k, vib0k, fi0k, fib0k, attk, attkv, vn0, vnk] = prealloc(len/nn, 3);  tk = vib0k(:,1);  timu = tk;
    k0 = fix(5/ts); % exculde the first 5s
    ki = timebar(nn, len, 'Initial align based on inertial frame & curve-fit.');
    G = glv.g0*eth.cl/glv.wie;
    xk = zeros(4,3); Pxk = diag([G*[1;1;1];10])^2;
    for k=1:nn:len-nn+1
        ...
        if k>k0
            ...
            qni0 = m2qua(Cni0);
            wt = tk(k1)*glv.wie; vib01 = ([sin(wt)-wt, 1-cos(wt), wt, 0]*xk)';
            qi0ib0 = dv2atti(vi0k(k1,:)', vi0, vib01, vib02-xk(end,:)');  % 28/01/2023
            qnb = qmul(qmul(qni0,qi0ib0),qib0b);
            attkv(ki,:) = q2att(qnb)';    % using vib0 fit
            vn0(ki,:) = -qmulv(qmul(qni00,qi0ib0),xk(end,:)');  vnk(ki,:) = qmulv(qmul(qni0,qi0ib0),rk);
...
%     attk(1:k0,:) = repmat(attk(k0+1,:),k0,1);
    ...
    res = varpack(lat, nts, vib0k, pib0k, fib0k, vi0k, pi0k, fi0k, attk, attkv, att0, vn0, vnk); 
    att0 = attkv(end,1:3)';
    resdisp('Initial align attitudes (arcdeg)', att0/glv.deg);
    if isfig, ai0plot(timu, attk, attkv, vn0, vnk); end
    
function ai0plot(t, attk, attkv, vn0, vnk)
global glv
    myfigure;
    ...
    subplot(222), plot(t, vn0), xygo('vn0 / m/s');
    subplot(224), plot(t, vnk), xygo('vnt / m/s');
