function Ft = etm_LSE(ins)
% SINS Error Transition Matrix (left SE(3) error model)
%
% Prototype: Ft = etm_LSE(ins)
% Input: ins - SINS structrue array
% Output: Ft - 15x15 error transition matrix
%
% See also  etm, kffk, kfc2d, kfupdate, insupdate.
...
%             0,           0,  0 ];
	...
	%%     fi     dvn    dpos    eb       db
    Cbn = ins.Cnb'; % Ref: Zhu T. Analysis and comparison of Euler angles based error model based on Lie groups of the SINS, 2022
	Ft = [ -askew(ins.wib)   -Cbn*Mav*ins.Cnb   Cbn*Map  -eye(3)  O33 
           -askew(ins.fb)   Cbn*(askew(ins.vn)*Mav-askew(ins.eth.wnie))*ins.Cnb-askew(ins.wib)   -Cbn*Mvp   O33   -eye(3) 
           O33    -Mpv*ins.Cnb    -Mpp     O33      O33
