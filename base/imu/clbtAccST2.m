function [Cba, Ka, Ka2, db, gS] = clbtAccST2(Acc, Cba0, g0, ang)
global glv
...
%         A(len+1,:) = [2*X(1:3); 0; 0; 0; 0;0;0]';
        A = [Acc(:,1:3), -Acc(:,4), abs(Acc(:,4)), ones(len,1), abs([Acc(:,1:3)*a, Acc(:,1:3)*b]).^2];
        A(len+1,:) = [2*X(1:3); 0; 0; 0; 0;0]';
        Y = -(A*X); Y(end)=0;
        dX = lscov(A, Y, W);
        X = X+dX;
    ...
    Cba = X(1:3); Ka = X(4); Ka2 = X(5); db = X(6);  gS = X(7:8);
	f = Ka*Acc(:,4);
% 	f = f - Ka2*abs(f) - db - [abs([Acc(:,1:3)*Cba, Acc(:,1:3)*a, Acc(:,1:3)*b])].^2*X(7:9);
	f = f - Ka2*abs(f) - db - [abs([Acc(:,1:3)*a, Acc(:,1:3)*b])].^2*gS;
	err = f - Acc(:,1:3)*Cba;
    myfig, 
